...
class Citizen:
    ...
    def add_subordinate(self, subordinate: Citizen) -> None:
        ...
        True
        """
        new = merge(self._subordinates, [subordinate])
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        if self.cid == cid:
            ...
            for subordinates in self._subordinates:
                return subordinates.get_citizen(cid)
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        if self._subordinates == []:
            ...
            new = self._subordinates
            for subordinate in self._subordinates:
                new = merge(subordinate.get_all_subordinates(), new)
            return new
        # Note: This method must call itself recursively
    ...
    def get_society_head(self) -> Citizen:
        ...
        if self._superior is None:
            ...
            top = self._superior.get_society_head()
            return top
        # Note: This method must call itself recursively
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        True
        """
        if self._superior is None or self.get_citizen(cid)._superior is None:
            return self
        elif self._superior == self.get_citizen(cid)._superior:
            return self._superior
        else:
            i = self.get_citizen(cid).get_superior().cid
            return self._superior.get_closest_common_superior(i)
        # Note: This method must call itself recursively
...
class Society:
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        if self._head.cid == cid:
            ...
            i = self._head.get_all_subordinates()
            for subordinates in i:
                if subordinates.cid == cid:
                    return subordinates
            return None
    ...
    def get_citizens_with_job(self, job: str) -> List[Citizen]:
        ...
        """
        new = []
        i = [self._head] + self._head.get_all_subordinates()
        for r in i:
            if r.job == job:
                new = merge(new, [r])
        return new
    ...
    def change_citizen_type(self, cid: int,
        ...
        if district_name is None:
            i = self.get_citizen(cid)
            new = Citizen(i.cid, i.manufacturer, i.model_year, i.job, i.rating)
            m = i.get_direct_subordinates()
            for e in m:
                new.become_subordinate_to(e)
            self.add_citizen(new, i.get_superior())
            if i.get_superior() is not None:
                i.get_superior().remove_subordinate(cid)
            return new
        else:
            i = self.get_citizen(cid)
            new = DistrictLeader(i.cid, i.manufacturer, i.model_year, i.job, i.rating, district_name)
            m = i.get_direct_subordinates()
            for e in m:
                new.become_subordinate_to(e)
            self.add_citizen(new, i.get_superior())
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        if self._head == superior:
            self._head = citizen
            lst = self._head.get_direct_subordinates()
            for i in lst:
                self._head.add_subordinate(i)
            self._head.add_subordinate(superior)
            if isinstance(citizen.get_superior(), Citizen):
                self.change_citizen_type(citizen.cid, superior.get_district_name())
            else:
                self.change_citizen_type(citizen.cid)
        else:
            superior.set_superior(citizen)
            lst = superior.get_direct_subordinates()
            for i in lst:
                self._head.add_subordinate(i)
...
class DistrictLeader(Citizen):
    ...
    def get_district_name(self) -> str:
        ...
        """
        return str(self._district_name)
